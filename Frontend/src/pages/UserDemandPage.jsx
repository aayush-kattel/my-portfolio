...
/* ═══════════════════════════════════════════════════════════════
   CV GENERATOR — lives here like OverviewPage lives in AdminDashboard
   ═══════════════════════════════════════════════════════════════ */
...
function initialsAvatar(name, accent) {
  ...
  return `data:image/svg+xml,${encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" width="110" height="110">
      ...
      <text x="55" y="70" text-anchor="middle" font-size="38" fill="${accent}"
        font-family="Georgia,serif" font-weight="600">${ini.toUpperCase()}</text>
    </svg>`
  )}`;
}
...
function CVPreview({ data, cvTheme, photoSrc }) {
  ...
  return (
    <div style={s.page} id="cv-preview-render">
      <div style={s.topLine}/>
      <div style={s.sb}>
        <div style={s.avWrap}><img style={s.avImg} src={photoSrc||initialsAvatar(data.name,t.accent)} alt={data.name}/></div>
        <div style={s.sbName}>
        ...
        {allSkills.length>0&&<><div style={s.div}/><div><div style={s.sbH3}>Core Skills</div><div style={s.tags}>{allSkills.map((sk,i)=><span key={i} style={s.tag}>{sk}</span>)}</div></div></>}
        {allTools.length>0&&<><div style={s.div}/><div><div style={s.sbH3}>Tools & Platforms</div><div style={s.tags}>{allTools.map((sk,i)=><span key={i} style={s.tag}>{sk}</span>)}</div></div></>}
      </div>
      <div style={s.main}>
        <div style={s.hero}>
        ...
        <div style={s.secs}>
          {data.profile&&<div><div style={s.secLbl}><span style={s.secNum}>01</span><span style={s.secTitle}>Profile</span><div style={s.secLine}/></div><div style={s.profCard}>{data.profile}</div></div>}
          {allEdu.length>0&&<div><div style={s.secLbl}><span style={s.secNum}>02</span><span style={s.secTitle}>Education</span><div style={s.secLine}/></div><div style={s.eduList}>{allEdu.map((e,i)=><div key={i} style={s.eduItem}><div style={s.eduDc}><div style={s.eduDot}/>{i<allEdu.length-1&&<div style={s.eduLn}/>}</div><div><div style={s.eduH4}>{e.degree}</div>{e.school&&<div style={s.eduSchool}>{e.school}</div>}{e.year&&<div style={s.eduYear}>{e.year}</div>}{e.desc&&<div style={s.eduDesc}>{e.desc}</div>}</div></div>)}</div></div>}
          {allProj.length>0&&<div><div style={s.secLbl}><span style={s.secNum}>03</span><span style={s.secTitle}>Projects</span><div style={s.secLine}/></div><div style={s.projGrid}>{allProj.map((p,i)=><div key={i} style={p.coming?s.projComing:s.projCard}><div style={{...s.projTitle,...(p.coming?{color:t.highlight}:{})}}>{p.title}</div>{p.stack&&<div style={{...s.projStack,...(p.coming?{color:t.highlight}:{})}}>{p.stack}</div>}{p.desc&&<div style={s.projDesc}>{p.desc}</div>}</div>)}</div></div>}
          <div>
            <div style={s.secLbl}><span style={s.secNum}>04</span><span style={s.secTitle}>Experience</span><div style={s.secLine}/></div>
            {hasExp
              ? <div style={s.eduList}>{allExp.map((e,i)=><div key={i} style={s.eduItem}><div style={s.eduDc}><div style={s.eduDot}/>{i<allExp.length-1&&<div style={s.eduLn}/>}</div><div><div style={s.eduH4}>{e.title}</div>{e.company&&<div style={s.eduSchool}>{e.company}</div>}{e.year&&<div style={s.eduYear}>{e.year}</div>}{e.desc&&<div style={s.eduDesc}>{e.desc}</div>}</div></div>)}</div>
              : <div style={s.expCard}><div style={s.expTitle}>Actively Seeking First Professional Role</div><div style={s.expDesc}>{data.expFallback||"Ready to bring fresh energy, strong work ethic, and modern technical skills to a professional team."}</div></div>
            }
          </div>
        </div>
        <div style={s.foot}>
...
/* ── CVGeneratorPage — the inner page component, no overlay, no fixed ── */
function CVGeneratorPage() {
  ...
  /* RCard — rainbow border card matching AdminDashboard's Card */
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",minHeight:0}}>
      ...
      {/* ── Sub-topbar: theme dots + preview/edit toggle + download ── */}
      <div style={{
        flexShrink:0, position:"sticky", top:0, zIndex:10,
        background:T("rgba(240,232,216,0.95)","rgba(20,21,22,0.95)"),
        borderBottom:`1px solid ${T("rgba(26,18,8,0.08)","rgba(255,255,255,0.07)")}`,
        padding:"8px 20px", display:"flex", alignItems:"center",
        gap:"12px", backdropFilter:"blur(12px)",
      }}>
        {/* cv: label */}
        ...
        <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
          ...
          {Object.entries(CV_THEMES).map(([key,th])=>(
            <button key={key} title={th.name} onClick={()=>setCvTheme(key)} style={{width:"14px",height:"14px",borderRadius:"50%",background:th.accent,border:cvTheme===key?"2.5px solid #fff":"2px solid transparent",cursor:"pointer",transform:cvTheme===key?"scale(1.25)":"scale(1)",transition:"transform .15s",outline:"none"}}/>
          ))}
        </div>
        ...
        {/* Preview / Edit toggle */}
        <button onClick={()=>setStep(s=>s==="form"?"preview":"form")} style={{display:"inline-flex",alignItems:"center",gap:"5px",padding:"5px 12px",borderRadius:"7px",border:"none",cursor:"pointer",background:step==="preview"?"linear-gradient(135deg,#5ba898,#4a9080)":T("rgba(26,18,8,0.08)","rgba(255,255,255,0.07)"),color:step==="preview"?"#fff":text,fontSize:"11px",fontFamily:"monospace",fontWeight:600,transition:"background .2s"}}>
          {step==="form"?<><FaEye size={11}/> Preview</>:<><FaPen size={10}/> Edit</>}
        </button>

        {step==="preview"&&(
          <button onClick={handleDownload} disabled={downloading} style={{display:"inline-flex",alignItems:"center",gap:"5px",padding:"5px 12px",borderRadius:"7px",border:"none",cursor:downloading?"wait":"pointer",background:"linear-gradient(135deg,#5ba898,#4a9080)",color:"#fff",fontSize:"11px",fontFamily:"monospace",fontWeight:600,opacity:downloading?0.7:1}}>
            ...
            <div>
              ...
              <RCard title="// experience">
                <div style={{fontSize:"10px",color:text2,fontFamily:"monospace",marginBottom:"12px",padding:"6px 10px",background:T("rgba(26,18,8,0.05)","rgba(255,255,255,0.04)"),borderRadius:"6px",border:`1px solid ${T("rgba(26,18,8,0.08)","rgba(255,255,255,0.06)")}`}}>
                  leave all fields empty → shows "seeking role" box · add entries → shows timeline
                </div>
                ...
                <CvBtn small onClick={()=>addItem("experience",{title:"",company:"",year:"",desc:""})}><FaPlus size={9}/> add experience</CvBtn>
              </RCard>
            </div>
          </div>
...
/* ═══════════════════════════════════════════════════════════════
   PAGES MAP — same pattern as AdminDashboard
   ═══════════════════════════════════════════════════════════════ */
const NAV = [
  { id: "cv", label: "CV Generator", Icon: FaFilePen },
...
/* ═══════════════════════════════════════════════════════════════
   MAIN UserDemandPage — mirrors AdminDashboard shell exactly
   ═══════════════════════════════════════════════════════════════ */
export default function UserDemandPage() {
  ...
  /* sudo close -userdemand typed anywhere on page */
  const [typed, setTyped] = useState("");
  const CLOSE_CMD = "sudo close -userdemand";
  useEffect(()=>{
    const h = e => {
      setTyped(prev=>{
        const next=(prev+e.key).slice(-CLOSE_CMD.length);
        if(next.endsWith(CLOSE_CMD)){ navigate("/"); return ""; }
        return next;
      });
    };
  ...
  return (
    ...
    >
      ...
      {sidebarOpen && window.innerWidth<=768 && (
        <div onClick={()=>setSidebarOpen(false)} style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",zIndex:99}}/>
      )}

      {/* ── Sidebar — exact copy of AdminDashboard sidebar ── */}
      <div style={{
        ...
        {/* Back to portfolio + command hint */}
        <div style={{padding:"10px 8px 18px"}}>
          <button onClick={()=>navigate("/")}
            title={!sidebarOpen?"Back to Portfolio":undefined}
            style={{width:"100%",display:"flex",alignItems:"center",gap:"12px",padding:"10px 10px",borderRadius:"10px",border:"none",cursor:"pointer",background:"transparent",color:t("rgba(26,18,8,0.45)","rgba(228,230,235,0.4)"),fontFamily:"monospace",fontSize:"12px",whiteSpace:"nowrap",overflow:"hidden",transition:"all .2s"}}
      ...
      {/* ── Main content — same as AdminDashboard ── */}
      <div style={{
        flex:1,
        marginLeft:window.innerWidth<=768?(sidebarOpen?`${ICON_W+SIDEBAR_W}px`:"0"):`${ICON_W+SIDEBAR_W}px`,
        ...
        {/* Page content — no extra padding wrapper, CVGeneratorPage owns its scroll */}
